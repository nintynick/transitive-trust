...
export const queriesRouter = router({
  ...
  getPersonalizedScore: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // Get endorsements for the subject
  ...
  // SearchSubjects with personalized ranking - Spec Section 8.1.2
  searchSubjects: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      // First, get subjects matching the search criteria
      const subjects = await searchSubjects(input.query || '', {
        domain: input.domain,
        location: input.location,
        limit: input.limit * 2, // Fetch more to filter after scoring
        offset: input.offset,
      });

      // Compute personalized scores for each subject
      ...
      // Filter by score and confidence thresholds
